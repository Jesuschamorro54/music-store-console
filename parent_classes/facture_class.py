...
class Facture:
    ...
    def update_stock(self, products, doc):
        self.file = open(f"/poo/music-store-console/files/stocktaking.json")
        data = self.file.read()
        data = data.split("\n")
            ...
            for key in products:
                for i in range(len(dictionary)):
                    if dictionary[i]["name"].lower() in key.lower():
                        dictionary[i]["lot"] += products[key]
        self.file.close()
        self.file = open(f"/poo/music-store-console/files/stocktaking.json", "w")
        self.file = open(f"/poo/music-store-console/files/stocktaking.json", "a+")
        for i in range(len(dictionary)):
